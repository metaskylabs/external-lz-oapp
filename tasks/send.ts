...
task('lz:oft:send', 'Sends tokens from either OFT or OFTAdapter')
  ...
  .setAction(async (taskArgs: Args, {ethers, deployments}) => {
    ...
    const oftDeployment = await deployments.get('LzBurnMintAdapter');
    ...
    const decimals = 18 //await oftContract.decimals();
    const amount = ethers.utils.parseUnits(taskArgs.amount, decimals);
    let options = Options.newOptions().addExecutorLzReceiveOption(165000, 0).toBytes();
